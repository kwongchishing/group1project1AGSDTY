using UnityEngine.AI;using UnityEngine;public class tungtung_pathfinding:MonoBehaviour{
    ...
    void Update(){
        if(Vector3.Distance(Player1.transform.position,transform.position)>4f&&Vector3.Distance(Player1.transform.position,transform.position)<=16f){
        ...
        if(Vector3.Distance(Player1.transform.position,transform.position)>16f){
